...
//
    ...
    BaseType_t xReturned = xTaskCreate(ir_decoder_task, "IR Decoder Task", 4096, NULL, IR_DECODER_TASK_PRIORITY, &ir_decoder_task_handle);
    ...
    xReturned = xTaskCreate(speaker_driver_play_task, "Speaker", 4096, NULL, SPEAKER_TASK_PRIORITY, &speaker_task_handle);
    if (xReturned != pdPASS) {
        ESP_LOGE(TAG, "Failed to create speaker task");
    } else {
        ESP_LOGI(TAG, "Speaker task created with priority %d", SPEAKER_TASK_PRIORITY);
    }
